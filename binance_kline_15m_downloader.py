...
class BinanceKline15mDownloader:
    def __init__(self):
    ...
    def download_historical_data(self, symbol, start_date, end_date, interval='15m'):
        """下载历史15分钟K线数据"""
        print(f"开始下载 {symbol} 的历史15分钟K线数据...")
        ...
        # 创建保存目录
        if not os.path.exists('kline_data_15m'):
            os.makedirs('kline_data_15m')
        
        while current_start < end_timestamp:
            # 计算当前批次的结束时间（最多1000条记录）
            # 15分钟间隔，1000条记录约为10.42天
            batch_end = min(current_start + (1000 * 15 * 60 * 1000), end_timestamp)
        ...
        # 保存到文件
        filename = f"kline_data_15m/{symbol}_{interval}_{start_date}_to_{end_date}.txt"
        ...
        # 显示价格范围统计
        if formatted_data:
            high_prices = [float(item['high_price']) for item in formatted_data]
            low_prices = [float(item['low_price']) for item in formatted_data]
            print(f"期间最高价: {max(high_prices)}")
            print(f"期间最低价: {min(low_prices)}")
...
def main():
    # 初始化下载器
    downloader = BinanceKline15mDownloader()
    ...
    try:
        ...
        print(f"数据包含完整的开盘价、最高价、最低价、收盘价等信息")
